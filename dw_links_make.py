...
import re
import time
import random # Random shuffle ke liye
import cloudinary
...
# --- Cloudinary Setup (Apni details check kar lijiye) ---
cloudinary.config( 
        ...
        try:
            ...
            async for message in client.iter_messages(channel, min_id=last_msg_id, reverse=True):
                ...
                if message.photo and message.text and "diskwala.com" in message.text:
                    ...
                    print(f"🔍 Processing Msg: {message.id}")
                    
                    # 1. Download & Upload to Cloudinary
                    ...
                    if new_link:
                        ...
                        # Poori list ko shuffle (mix) karein
                        ...
                        count += 1
                        print(f"✅ Saved & Shuffled ({count}) in {current_file}")
